...
1. kafka架构是什么样的
2. 什么是多副本机制，为什么要有多副本机制
3. zookeeper和kraft的作用

## kafka架构

### 基础架构

![image-20250928214952191](../图片/消息队列/kafka总架构.png)

1. broker可以理解为kafka本身负责接收生产者消息、持久化到磁盘、
2. zookeeper相当于消息中心，保存topic和partition的路由通信（新版的`kafka`使用kraft代替zk作为注册中心）

### topic

![image-20250928215641323](../图片/消息队列/topic.png)

从消息的角度来看：

1. broker通过topic对**不同类型**的消息进行分类
2. 生产者把不同类型的消息发送给对应的topic

### partition

![image-20250928223602124](../图片/消息队列/partition.png)

从队列的角度来看：

1. partition就是消息队列中的队列
2. 一个消息可以被多个消费者消费，为了记录每个消费者消费到哪里，partition中每个消息都有一个唯一标识offset（偏移量）

kafka只能保证**单个**partition中的消息是有序的

### offset

你可以把Kafka的一个 **`Partition`**（分区）想象成**一本很厚的书**。

*   **这本书（Partition）**：里面按顺序写满了消息（故事内容）。
*   **每条消息**：就是书里的**一句话**。
*   **`offset`（偏移量）**：就是这本书的**页码**。
...
1.  **`offset` 是什么？**
    *   它就是每个消息在分区（Partition）这个“书”里的**唯一、连续、递增的页码**。
    *   比如第一条消息在 `offset=0`，第二条在 `offset=1`，以此类推。**这个页码从0开始，只增不减。**

2.  **`offset` 的核心作用是什么？**
    *   **记录消费者的阅读进度**。
    *   每个消费者（读者）都**自己记住自己读到了哪一页**（例如，读到第105页了）。
    *   当这个消费者下次再打开这本书时，它不用从头开始读，而是**直接翻到第105页，继续往下读**。
    *   这样，即使消费者程序重启，它也不会丢失进度，也不会重复阅读已经读过的内容。

3.  **关键特性：**
    *   **由消费者自己管理**：Kafka不跟踪每个消费者的具体进度，它只负责存消息。进度（读到哪个offset了）由消费者自己提交到Kafka的一个特殊主题里。这给了消费者极大的灵活性。
    *   **每个消费者组都有自己的进度**：就像同一本书，你和你朋友可以各自用不同的书签，记录自己读到的位置，互不干扰。

---

**总结**

所以，**`offset` 就是Kafka用来在数据流中给消息编号的“页码”，它的唯一使命就是让消费者知道自己消费到哪儿了，从而实现精确的进度跟踪和故障恢复。**

## 保证高可用

### 分布式

![image-20250928225522582](../图片/消息队列/性能.png)

在单机上，IO将成为某个topic的性能瓶颈，为了解决这个问题：

1. 每个partition只存一部分消息
1. 一个topic下的不同partition可以分布到不同的broker节点上

### 多副本机制

每个partition的数据都会同步到其他broker节点上，
